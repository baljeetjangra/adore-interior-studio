...
const ServicesThreeElements = () => {
  const setupScene = (scene: THREE.Scene, camera: THREE.Camera) => {
    ...
    const stoveMaterial = new THREE.MeshBasicMaterial({
      color: 0x333333,
      wireframe: true,
    });
    ...
    const coffeeTableMaterial = new THREE.MeshBasicMaterial({
      color: 0x333333,
      wireframe: true,
    });
    ...
    const nightstandMaterial = new THREE.MeshBasicMaterial({
      color: 0x333333,
      wireframe: true,
    });
  ...
  const animate = (scene: THREE.Scene) => {
    if (scene.userData.serviceIcons) {
      ...
      scene.userData.serviceIcons.forEach(
